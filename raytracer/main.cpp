...
Scene *initScenePerso()
{
    Scene *scene = initScene();
    setCamera(scene, point3(-10, 20, 5), vec3(0, 0, 0), vec3(0, 1, 0), 60, (float)WIDTH / (float)HEIGHT);
    ...
    mat.diffuseColor = color3(0.8);

    Mesh *mesh = new Mesh(mat);
    Importer *importer = new Importer(mesh);
    importer->loadObjFile("Models/cube.obj");

    addMesh(scene, *mesh);

    addLight(scene, initLight(point3(0, 20, 0), color3(1, 1, 1)));
    addLight(scene, initLight(point3(5, 20, 5), color3(1, 1, 1)));
